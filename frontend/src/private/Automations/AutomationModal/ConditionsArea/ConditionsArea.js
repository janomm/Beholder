...
function ConditionsArea(props) {
    ...
    useEffect(() => {
        setConditons(parseConditions(props.conditions))
    }, [props.conditions])
    ...
    function onAddConditionClick(event) {
        const parsedConditions = parseConditions(event.target.value.eval)[0];
        if (conditions.some(c => c.eval === parsedConditions.eval)) return;
        ...
        <React.Fragment>
            <div className="row">
                <div className="col-12 mb-3">
                    ...
                    <VariableInput indexes={indexes} onAddClick={onAddConditionClick} onChange={onKeySelectChange} selectedIndex={selectedIndex} />
                </div>
            </div>
            {
                conditions
